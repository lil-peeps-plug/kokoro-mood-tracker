  ...
  return (
    <div className="music" ref={rootRef}>
      ...
      >
        <button
          type="button"
          className="music-menu__toggle"
          role="menuitemcheckbox"
          aria-checked={playing}
          onClick={toggle}
        >
          <span className="music-menu__toggle-icon" aria-hidden="true">
            {playing ? '❚❚' : '▶'}
          </span>
          <span>{triggerLabel}</span>
        </button>

        <label className="music-menu__slider-row">
          <span className="music-menu__slider-label">{t.musicVolume}</span>
          <input
            className="music-menu__slider"
            type="range"
            min={0}
            max={100}
            step={1}
            value={volume}
            onChange={(e) =>
              setVolume(Number.parseInt(e.currentTarget.value, 10))
            }
